...
for i in range(160):
    ...
    #img=Image.open(dataset_path1+'/'+str(i+1)+'.jpg')
    #import cv2
    #cv2.resize(img, (80,70))
    
    #img.resize((80,70))
    #print(img.shape)
    #img = imread(str(i+1)+'.jpg')
...
def recogniser(img_number):
    ...
    plt.title('Input:'+str(img_number+1), fontdict = {'fontsize' : 60})
    plt.subplots_adjust(left = 15, right=20, top=20) #1.2, 2.5
    plt.tick_params(labelleft=False, labelbottom=False, bottom=False,top=False,right=False,left=False, which='both')
    count+=1
...
#     print(set_number)
    ...
    if norms[index] < t0:
        if(index>=(6*set_number) and index<(6*(set_number+1))):
            plt.title('Matched with:'+str(index+1), color='g', fontdict = {'fontsize' : 60})
            plt.imshow(training_tensor[index,:].reshape(height,width), cmap='gray')
            correct_pred += 1
        else:
            plt.title('Matched with:'+str(index+1), color='r', fontdict = {'fontsize' : 60})
            plt.imshow(training_tensor[index,:].reshape(height,width), cmap='gray')
            false_pos +=1
    else:
        if(img_number>=40):
            plt.title('Unknown face!', color='g', fontdict = {'fontsize' : 60})
            correct_pred += 1
        else:
            plt.title('Unknown face!', color='r', fontdict = {'fontsize' : 60})
            false_neg +=1
    plt.subplots_adjust(left = 15, right=20, top=20)
    plt.tick_params(labelleft=False, labelbottom=False, bottom=False,top=False,right=False,left=False, which='both')
    count+=1
